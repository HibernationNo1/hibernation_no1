...
from sub_module.mmdet.modules.base.module import BaseModule, ModuleList
from sub_module.mmdet.modules.utils.cnn import ConvModule
from sub_module.mmdet.modules.utils.upsample import ConvTranspose2d, CARAFEPack
...
class FCNMaskHead(BaseModule):
    ...
    def __init__(self,
        ...
        loss_mask_type = loss_mask.pop('type')
        if loss_mask_type == 'CrossEntropyLoss':
            self.loss_mask = CrossEntropyLoss(**loss_mask)
        else:
            raise TypeError(f"Target of `loss_mask` is 'CrossEntropyLoss'")
        ...
        if self.upsample_method is None:
            ...
            upsample_cfg_type = upsample_cfg_.pop('type', None)
            if upsample_cfg_type == 'deconv':
                self.upsample = ConvTranspose2d(**upsample_cfg_)
            else:
                raise TypeError(f"Target of `upsample_method` in `deconv` mode is 'ConvTranspose2d'")
            ...
            upsample_cfg_type = upsample_cfg_.pop('type', None)
            if upsample_cfg_type == 'carafe':
                self.upsample = CARAFEPack(**upsample_cfg_)
            else:
                raise TypeError(f"Target of `upsample_method` in `carafe` mode is 'CARAFEPack'")
                        
        elif self.upsample_method in ['nearest', 'bilinear']:
            # suppress warnings
            align_corners = (None
            ...
            upsample_cfg_type = upsample_cfg_.pop('type', None)
            self.upsample = nn.Upsample(**upsample_cfg_)
        ...
        predictor_cfg_type = self.predictor_cfg.pop('type')
        if predictor_cfg_type == 'Conv':
            self.conv_logits = nn.Conv2d(logits_in_channel, out_channels, 1)
        else:
            raise TypeError(f"Target of `conv_logits` is 'Conv2d'")
