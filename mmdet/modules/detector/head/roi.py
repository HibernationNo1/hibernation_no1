...
# from sub_module.mmdet.modules.register_module import ROI_HEADS
...
@ROI_HEADS.register_module()
class StandardRoIHead(BaseRoIHead, BBoxTestMixin_ROI, MaskTestMixin_ROI):
    ...
    def init_assigner_sampler(self):
        ...
        if self.train_cfg:
            ...
            if assigner_type == 'MaxIoUAssigner':
                ...
                TypeError(f"`bbox_assigner` only implemented 'MaxIoUAssigner'")
            ...
            if sampler_type == 'RandomSampler':
                self.bbox_sampler = RandomSampler(**sampler_cfg)
            else:
                raise TypeError(f"Target of `bbox_sampler` is 'RandomSampler'")

    def init_bbox_head(self, bbox_roi_extractor, bbox_head):
        """Initialize ``bbox_head``"""
        
        bbox_roi_extractor_type = bbox_roi_extractor.pop('type')
        if bbox_roi_extractor_type == 'SingleRoIExtractor':
            self.bbox_roi_extractor = SingleRoIExtractor(**bbox_roi_extractor)
        else:
            raise TypeError(f"`bbox_roi_extractor` only implemented 'SingleRoIExtractor'")
        
        bbox_head_type = bbox_head.pop('type')
        if bbox_head_type == 'Shared2FCBBoxHead':
            self.bbox_head = Shared2FCBBoxHead(**bbox_head)
        else:
            raise TypeError(f"`bbox_head` only implemented 'Shared2FCBBoxHead'")
    ...
    def init_mask_head(self, mask_roi_extractor, mask_head):
        ...
        if mask_roi_extractor is not None:
            ...
            mask_roi_extractor_type = mask_roi_extractor.pop('type')
            if mask_roi_extractor_type == 'SingleRoIExtractor':
                self.mask_roi_extractor = SingleRoIExtractor(**mask_roi_extractor)
            else:
                raise TypeError(f"`mask_roi_extractor` only implemented 'SingleRoIExtractor'")
            self.share_roi_extractor = False
        else:
            self.share_roi_extractor = True
            self.mask_roi_extractor = self.bbox_roi_extractor
        
        mask_head_type = mask_head.pop('type')
        if mask_head_type == 'FCNMaskHead':
            self.mask_head = FCNMaskHead(**mask_head)
        else:
            raise TypeError(f"`mask_head` only implemented 'FCNMaskHead'")
