...
class BBoxHead(BaseModule):
    ...
    def __init__(self,
        ...
        if self.with_cls:
            ...
            cls_predictor_type = self.cls_predictor_cfg.get('type', None)
            if cls_predictor_type == 'Linear':
                self.fc_cls = nn.Linear(in_features=in_channels,
                                        out_features=cls_channels)
            else:
                raise TypeError(f"Target of `fc_cls` is 'nn.Linear'")
        if self.with_reg:
            out_dim_reg = 4 if reg_class_agnostic else 4 * num_classes
            
            reg_predictor_type = self.reg_predictor_cfg.get('type', None)
            if reg_predictor_type == 'Linear':
                self.fc_reg = nn.Linear(in_features=in_channels,
                                        out_features=cls_channels)
            else:
                raise TypeError(f"Target of `fc_reg` is 'nn.Linear'")
...
class ConvFCBBoxHead(BBoxHead):
    ...
    def __init__(self,
        ...
        if self.with_reg:
            ...
            reg_predictor_type = self.reg_predictor_cfg.get('type', None)
            if reg_predictor_type == 'Linear':
                self.fc_reg = nn.Linear(in_features=self.reg_last_dim,
